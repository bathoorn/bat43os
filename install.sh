...
if [ -d "bat43os" ]; then
  echo "bat43OS exists, backing up to .config/bat43os-backups folder."
  if [ -d ".config/bat43os-backups" ]; then
    echo "Moving current version of bat43OS to backups folder."
    mv "$HOME"/zaneyos .config/bat43os-backups/"$backupname"
    sleep 1
  else
    echo "Creating the backups folder & moving bat43OS to it."
    mkdir -p .config/bat43os-backups
    mv "$HOME"/zaneyos .config/bat43os-backups/"$backupname"
    sleep 1
  fi
else
  echo "Thank you for choosing bat43OS."
  echo "I hope you find your time here enjoyable!"
fi

echo "-----"

echo "Cloning & Entering bat43OS Repository"
...
sudo nixos-rebuild switch --flake ~/bat43os/#${profile}
